...
static void free_platform_resource(void)
{
	if(gpio_addr) {
		iounmap(gpio_addr);
	}
...
static int init_platform_resource(void)
{
	...
	if(!gpio_wakeup_hdle) {
		pr_warning("%s: tp_wakeup request gpio failed!\n", __func__);
	}

	gpio_reset_hdle = gpio_request_ex("ctp_para", "ctp_reset");
	if(!gpio_reset_hdle) {
		pr_warning("%s: tp_reset request gpio failed!\n", __func__);
	}

	gpio_int_hdle = gpio_request_ex("ctp_para", "ctp_int_port");
	if(!gpio_int_hdle) {
		pr_warning("%s: tp_int request gpio failed!\n", __func__);
		gpio_int_hdle = 0;
	}
...
static void edt_ft5x06_ts_clear_penirq(int irqnum)
{
	int reg_val;

	reg_val = readl(gpio_addr + PIO_INT_STAT_OFFSET);

	if((reg_val = (reg_val & (1 << irqnum)))) {
		writel(reg_val,gpio_addr + PIO_INT_STAT_OFFSET);
	}
	return;
}

static int edt_ft5x06_ts_judge_int_occur(int irqnum)
{
	int reg_val;
	int ret = -1;
...
#if defined(CONFIG_ARCH_SUN5I)
	const struct edt_ft5x06_platform_data *pdata = dev->platform_data;
#endif
	u8 cmd = 0xf9;
	u8 rdbuf[26];
	int i, type, x, y, id;
...
#if defined(CONFIG_ARCH_SUN5I)
	if(edt_ft5x06_ts_judge_int_occur(pdata->irq_pin) != 0) {
		goto out;
	}
	edt_ft5x06_ts_clear_penirq(pdata->irq_pin);
#endif
...
#if defined(CONFIG_ARCH_SUN5I)
static int __devinit edt_ft5x06_ts_config_irq(int irq_number, ext_int_mode int_mode)
{
	int reg_val;
	int reg_num  = irq_number % 8;
	int reg_addr = irq_number / 8;
	...
	edt_ft5x06_ts_clear_penirq(irq_number);

	reg_val = readl(gpio_addr + PIO_INT_CTRL_OFFSET);

	reg_val |= (1 << irq_number);
...
static int __devinit edt_ft5x06_ts_wakeup(struct i2c_client *client,
					 int wakeup_handle)
{
	if(wakeup_handle) {
		if(EGPIO_SUCCESS != gpio_write_one_pin_value(wakeup_handle, 0, "ctp_wakeup")){
			pr_info("%s: err when operate gpio. \n", __func__);
		}
		mdelay(TS_WAKEUP_LOW_PERIOD);
		if(EGPIO_SUCCESS != gpio_write_one_pin_value(wakeup_handle, 1, "ctp_wakeup")){
			pr_info("%s: err when operate gpio. \n", __func__);
		}
					 ...
					 int reset_handle)
{
	if(reset_handle) {
		...
		if(EGPIO_SUCCESS != gpio_write_one_pin_value(reset_handle, 0, "ctp_reset")) {
			pr_info("%s: err when operate gpio. \n", __func__);
		}
		mdelay(TS_RESET_LOW_PERIOD);
		if(EGPIO_SUCCESS != gpio_write_one_pin_value(reset_handle, 1, "ctp_reset")) {
			pr_info("%s: err when operate gpio. \n", __func__);
		}
...
#if defined(CONFIG_ARCH_SUN5I)
	init_platform_resource();

	edt_ft5x06_ts_reset(client, gpio_reset_hdle);
	edt_ft5x06_ts_wakeup(client, gpio_wakeup_hdle);

	edt_ft5x06_ts_config_irq(pdata->irq_pin, NEGATIVE_EDGE);
#else
...
static int __init edt_ft5x06_ts_init(void)
{
    ...
    ret = i2c_add_driver(&edt_ft5x06_ts_driver);
    return ret;
